...
export class TerminalService {
  ...
  init(): void {
    ...
    logger.info('[TerminalService] Initialized (cleanup timer started)');
  }
  ...
  shutdown(): void {
    ...
    logger.info('[TerminalService] Shutdown complete (all sessions closed)');
  }
  ...
  sendData(sessionId: string, data: string): { success: boolean; reason?: string } {
    const session = activeSessions.get(sessionId);
    if (!session) {
