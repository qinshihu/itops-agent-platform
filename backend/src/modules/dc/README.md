...
> **最后刷新**：2026-07-23（slots routes 业务规则下沉到 dcSlotService + 全部 RBAC + catch logger 补齐）
...
│   ├── index.ts                # 13 子路由聚合
│   ├── rooms/racks/slots/lifecycle/pdus/overview/devices
│   ├── export/import/manufacturers/device-types
...
└── services/                       # 4 业务服务（含 2 个测试文件）
    ...
    ├── dcSlotService.ts            ← U 位分配/迁位/移除业务规则（routes/slots.ts 已下沉）
    └── dcCrudService.ts            ← routes→service 抽象门面（2026-07-23 新增 slotsBusiness 入口）
```
...
- **slots routes 业务规则下沉**：`routes/slots.ts` 之前内联了一份 U 位冲突/容量/设备型号 u_height 继承/生命周期写入的副本，与 `dcSlotService.ts`（已有完整实现）不同步。修复后 routes/slots.ts 调用 `dcCrudService.slotsBusiness.assignSlot/moveSlot/removeSlot`，catch `DcSlotBusinessError` 转 statusCode（409/400/404）。
- **13 个 routes 文件全部加 RBAC**：GET 开放；POST/PUT → `requireRole('admin', 'operator')`；DELETE → `requireRole('admin')`。README 第 27 行声明的 RBAC 现在真正生效（之前 0 处 requireRole）。
- **13 个 routes 文件 catch 块全部加 logger.error**：原 0 处 `logger.error` 调用，运维排障零线索。统一标签：`Failed to operate dc ${filename}`。
- **dcCrudService 新增 `slotsBusiness: dcSlotService` 暴露**：让 routes 有了真正的业务入口（之前 dcSlotService 模块未在 routes 中可见）。
- **index.ts 暴露 services**：除 routes 外，`export { dcCrudService, dcSlotService }` 方便后台 worker/scheduler 调用。
