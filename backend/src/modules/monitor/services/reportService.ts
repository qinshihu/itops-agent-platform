/**
 * 报告生成服务（PDF/Markdown/Word 模板 + 调度 + 生成）
 *
 * 模块归属：P1-6 infra 按子域拆分阶段 9（2026-07-07），从 `modules/infra/` 迁到 `modules/monitor/`
 * 原位置：modules/infra/services/reportService.ts
 * 原位置理由：报告生成在 v3 早期归到 infra 共享层，但实际上与 monitor 的报表/Dashboard 紧密耦合
 * 新归属理由：作为 monitor "报表" 子域的一部分
 */
